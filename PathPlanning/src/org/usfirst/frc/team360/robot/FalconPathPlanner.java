...
public class FalconPathPlanner
{
	...
	double[][] distance(double[][] smoothPath, double timeStep){
		double[][] distance = new double[smoothPath.length][2];
		distance[0][0]=0;
		distance[0][1]=0;
		for(int i=1; i<smoothPath.length; i++)
		{
			distance[i][0]=distance[i-1][0]+timeStep;
			distance[i][1] = Math.sqrt(Math.pow((smoothPath[i][0]-smoothPath[i-1][0]), 2) + Math.pow((smoothPath[i][1]-smoothPath[i-1][1]), 2)) + distance[i-1][1];
		}
		return distance;
	...
	*/
	...
	double[][] velocity(double[][] smoothPath, double timeStep){
		...
		{
			velocity[i][0]=velocity[i-1][0]+timeStep;
			velocity[i][1] = (smoothPath[i][1]-smoothPath[i-1][1])/timeStep;
		}
		return velocity;
	}
	double[][] acceleration(double[][] smoothPath, double timeStep){
		double[][] acceleration = new double[smoothPath.length][2];
		acceleration[0][0]=0;
		acceleration[0][1]=0;
		for(int i=1; i<smoothPath.length; i++)
		{
			acceleration[i][0] = acceleration[i-1][0]+timeStep;
			acceleration[i][1] = (smoothPath[i][1]-smoothPath[i-1][1])/timeStep;
		}
		return acceleration;
	}
	double[][] jerk(double[][] smoothPath, double timeStep){
		double[][] jerk = new double[smoothPath.length][2];
		jerk[0][0]=0;
		jerk[0][1]=0;
		for(int i=1; i<smoothPath.length; i++)
		{
			jerk[i][0] = jerk[i-1][0]+timeStep;
			jerk[i][1] = (smoothPath[i][1]-smoothPath[i-1][1])/timeStep;
		}
		return jerk;
	}
	/**
 ...
 */
	...
	public void calculate(double totalTime, double timeStep, double robotTrackWidth)
	{
		...
		centerDistance = distance(smoothPath, timeStep);
		rightDistance = distance(rightPath, timeStep);
		leftDistance = distance(leftPath, timeStep);

		origCenterVelocity = velocity(centerDistance, timeStep);
		origRightVelocity = velocity(rightDistance, timeStep);
		origLeftVelocity = velocity(leftDistance, timeStep);
